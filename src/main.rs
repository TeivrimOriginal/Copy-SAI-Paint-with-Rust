...
use app::{App, CanvasOp, DialogMode, FileDialog, PathKind};use renderer::Renderer;
...
fn main() {
    ...
    if let Some(p) = std::env::args().nth(1) {
        ...
        let r = if p.to_lowercase().ends_with(crate::project::EXT_DOT) {
            app.open_project(&p)
        } else {
            app.open_png(&p)
        };
    ...
    'main: while !window.should_close() {
        ...
        if !ui.consumed_click && !modal {
            ...
            app.cursor_in_canvas = inside
                && !matches!(app.tool, crate::tools::Tool::Eyedropper | crate::tools::Tool::Pan)
            ...
            if panning {
                ...
                if inside && (left || right) {
                    if pressed_l || pressed_r {
                        ...
                        if grab != crate::app::TransformGrab::None {
                            app.transform_grab_begin(cp, grab);
                            transform_dragging = true;
                        }
                    }
                    if transform_dragging {
                        app.transform_drag(cp, app.transform.as_ref().map(|t| t.grab).unwrap_or(crate::app::TransformGrab::None));
                    }
            ...
            if inside && (drawing_left || drawing_right) {
                app.move_select(cp);
            } else if app.floating.is_some() && app.sel_drag == crate::app::SelDrag::None {
                app.move_select(cp);
            }
