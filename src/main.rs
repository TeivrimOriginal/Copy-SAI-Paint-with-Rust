...
fn main() {
    ...
    while i < args.len() {
        ...
        if a == "--new" {
            ...
            // Проект .tpaint открывается целиком, PSD — своей картинкой,
            // остальное — как картинка.
            let low = a.to_lowercase();
            let r = if low.ends_with(tpaint::project::EXT_DOT) {
                app.open_project(a)
            } else if low.ends_with(".psd") {
                app.open_psd(a)
            } else {
                app.open_png(a)
            };
    ...
    // Файла не передавали — попробуем поднять автосохранение.
    if !got_file && App::autosave_pending() {
        if let Err(e) = app.restore_autosave() {
            app.notify(&format!("Автосохранение не открылось: {}", e));
        }
    }
    ...
    'main: while !window.should_close() {
        ...
        last_t = now;
        // Автосохранение: раз в минуту, если документ менялся.
        if app.autosave_due() && app.autosave_now() {
            app.notify("Автосохранено");
        }
            ...
            if let Some((path, mode)) = d.result.take() {
                ...
                if !path.is_empty() {
                    ...
                    let r = match mode {
                        ...
                        DialogMode::OpenPsd => app.open_psd(&path),
                        DialogMode::ExportPsd => app.export_psd(&path),
                    };
                    if let Err(e) = r {
...
fn apply(app: &mut App, a: Action) {
    ...
    Action::OpenBalance => app.open_balance(),
        ...
        Action::GlowFx => app.toggle_fx_glow(),
