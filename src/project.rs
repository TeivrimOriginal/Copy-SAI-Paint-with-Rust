...
pub fn save(doc: &Document, path: &str) -> Result<(), String> {
    ...
    for layer in &doc.layers {
        ...
        if VERSION >= 3 {
            ...
            w.u8(layer.meta.clipped as u8);
        }
        // С версии 4: метка цвета и параметры эффектов слоя.
        if VERSION >= 4 {
            w.u8(layer.meta.tag);
            let fx = &layer.meta.fx;
            w.u8(fx.shadow as u8);
            w.f32(fx.shadow_dx);
            w.f32(fx.shadow_dy);
            w.f32(fx.shadow_blur);
            w.f32(fx.shadow_opacity);
            w.bytes(&fx.shadow_color);
            w.u8(fx.outline as u8);
            w.f32(fx.outline_size);
            w.bytes(&fx.outline_color);
            // С версии 5: свечение слоя.
            if VERSION >= 5 {
                w.u8(fx.glow as u8);
                w.f32(fx.glow_blur);
                w.f32(fx.glow_opacity);
                w.bytes(&fx.glow_color);
            }
        }
    }
...
pub fn load(path: &str) -> Result<Document, String> {
    ...
    for _ in 0..count {
        ...
        if version >= 3 {
            ...
            clipped = if version >= 3 { r.u8()? != 0 } else { false };
        }
        // Эффекты слоя и метка цвета — с версии 4.
        let mut tag = 0u8;
        let mut fx = crate::doc::LayerFx::default();
        if version >= 4 {
            tag = r.u8()?.min(6);
            fx.shadow = r.u8()? != 0;
            fx.shadow_dx = r.f32()?.clamp(-512.0, 512.0);
            fx.shadow_dy = r.f32()?.clamp(-512.0, 512.0);
            fx.shadow_blur = r.f32()?.clamp(0.0, 200.0);
            fx.shadow_opacity = r.f32()?.clamp(0.0, 1.0);
            fx.shadow_color = r.take(4)?.try_into().unwrap_or([0, 0, 0, 255]);
            fx.outline = r.u8()? != 0;
            fx.outline_size = r.f32()?.clamp(0.5, 100.0);
            fx.outline_color = r.take(4)?.try_into().unwrap_or([0, 0, 0, 255]);
            // Свечение — с версии 5.
            if version >= 5 {
                fx.glow = r.u8()? != 0;
                fx.glow_blur = r.f32()?.clamp(0.0, 200.0);
                fx.glow_opacity = r.f32()?.clamp(0.0, 1.0);
                fx.glow_color = r.take(4)?.try_into().unwrap_or([255, 255, 255, 255]);
            }
        }
        layers.push(Layer {
            meta: LayerMeta {
