...
pub const UNIT_MASKS: u8 = 7;
/// Затемнение всего, что вне выделения: шейдер берёт альфу из маски выделения.
pub const UNIT_SEL: u8 = 8;
...
void main() {
    ...
    if (u_alpha_mask > 1.5) {
        // Маска выделения: гасим всё, что ВНЕ выделения. Так затемнение
        // идёт по настоящей форме области, а не по её прямоугольнику.
        float outside = 1.0 - t.r;
        frag = vec4(v_color.rgb, v_color.a * outside);
    } else if (u_alpha_mask > 0.5) {
        // Атлас глифов: одноканальное покрытие умножаем на прозрачность цвета.
        if (t.r < 0.004) { discard; }
        // Гамма-коррекция: без неё тонкие штрихи 12-14 px, нарисованные без
        // хинтинга, расплываются в серое пятно.
        float cov = pow(t.r, 0.72);
...
impl Renderer {
    pub fn new(width: i32, height: i32) -> Self {
        ...
        unsafe {
            ...
            let mut tex_masks = 0;
            ::gl::GenTextures(1, &mut tex_masks);
            let mut tex_sel = 0;
            ::gl::GenTextures(1, &mut tex_sel);
    ...
    /// Маска выделения: белая там, где выделено. Затемнение берётся из неё
    /// обратной стороной, поэтому форма выделения может быть любой.
    fn init_sel(&self) {
        unsafe {
    ...
    /// Загружает маску выделения (0..255 на пиксель холста).
    pub fn update_sel(&mut self, w: usize, h: usize, data: &[u8]) {
        unsafe {
            ::gl::BindTexture(::gl::TEXTURE_2D, self.tex_sel);
            ::gl::PixelStorei(::gl::UNPACK_ALIGNMENT, 1);
            ::gl::TexImage2D(
    ...
    /// Атлас масок слоёв: те же ячейки, но в оттенках серого.
    fn init_masks(&self) {
        unsafe {
    ...
    /// Загружает атлас масок слоёв (ячейки в сером, альфа = покрытие маски).
    pub fn update_masks(&mut self, rgba: &[u8]) {
        unsafe {
            ::gl::BindTexture(::gl::TEXTURE_2D, self.tex_masks);
            ::gl::TexImage2D(
                ::gl::TEXTURE_2D, 0, ::gl::RGBA8 as i32, THUMB_ATLAS as i32, THUMB_ATLAS as i32, 0,
    ...
    pub fn draw(&mut self, items: &[Item], tris: &[Tri], w: i32, h: i32) {
        ...
        for &unit in PASSES.iter() {
            let tex = match unit {
                UNIT_BG | UNIT_SOLID | UNIT_TOP => self.tex_white,
                UNIT_CHECKER => self.tex_pattern,
                UNIT_CANVAS => self.tex_canvas,
                UNIT_THUMBS => self.tex_thumbs,
                UNIT_MASKS => self.tex_masks,
                UNIT_SEL => self.tex_sel,
                UNIT_FLOAT => self.tex_float,
                _ => self.tex_atlas,
            };
            let alpha_mask = match unit {
                UNIT_ATLAS => 1.0,
                UNIT_SEL => 2.0,
                _ => 0.0,
            };
