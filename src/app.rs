...
impl App {
    ...
    /// Открывает PSD: получаем итоговую картинку документа и делаем новый
    /// документ в её размере. Слои самого PSD не читаются — ровно как
    /// показывает Photoshop при открытии такого файла.
    pub fn open_psd(&mut self, path: &str) -> Result<(), String> {
        self.end_stroke();
        let bytes = std::fs::read(path).map_err(|e| e.to_string())?;
        let img = crate::psd::read(&bytes)?;
        ...
        doc.layers[0].meta.name = if name.is_empty() { "PSD".to_string() } else { name };
        doc.layers[0].pixels.copy_from_slice(&img.pixels);
        doc.touch();
        self.doc = doc;
        self.history = History::new(40);
        ...
        self.selection = None;
        self.floating = None;
        self.notify(&format!("Открыт PSD {}×{}", img.width, img.height));
        Ok(())
    }

    /// Экспорт композиции всех слоёв в PSD (плоский, с альфой).
    pub fn export_psd(&mut self, path: &str) -> Result<(), String> {
        self.doc.ensure_composite();
        let (w, h) = (self.doc.width, self.doc.height);
        let bytes = crate::psd::write(w, h, &self.doc.composite);
        std::fs::write(path, bytes).map_err(|e| e.to_string())?;
        self.notify(&format!("Экспорт в PSD готов: {}×{}", w, h));
        Ok(())
    }
