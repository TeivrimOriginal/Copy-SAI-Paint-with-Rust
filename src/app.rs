...
impl App {
    ...
    pub fn move_stroke(&mut self, p: (f32, f32), secondary: bool) {
        ...
        if self.mask_base.is_some() {
            ...
            let shape = self.brush_shape();
            if let Some(m) = self.doc.active_layer_mut().mask.as_mut() {
                raster::mask_line_shape(m, w, h, self.smooth.0, self.smooth.1, s.0, s.1, r, hard, value, op, shape);
            }
            self.smooth = s;
            self.stroke_last = p;
            self.doc.touch();
        ...
        if tool.is_freehand() {
            ...
            let shape = self.brush_shape();
            let layer = self.doc.active_layer_mut();
            raster::brush_line_shape(layer, w, h, self.smooth.0, self.smooth.1, s.0, s.1, p2.0, p2.1, color, p2.2, shape);
            if mirror {
                ...
                raster::brush_line_shape(layer, w, h, m0.0, m0.1, m1.0, m1.1, p2.0, p2.1, color, p2.2, shape);
            }
            self.smooth = s;
