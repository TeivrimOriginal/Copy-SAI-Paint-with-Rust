...
pub enum DialogMode {
    ...
    ExportPng,
    ExportPngAlpha,
    /// Экспорт в JPEG: один кадр композиции, прозрачность заливается белым.
    ExportJpeg,
    /// Открытие PSD — импортируется итоговая картинка документа.
    OpenPsd,
    /// Экспорт композиции в PSD.
    ExportPsd,
}
...
impl DialogMode {
    pub fn title(self) -> &'static str {
        match self {
            ...
            DialogMode::OpenPsd => "Открыть PSD",
            DialogMode::ExportPsd => "Экспорт в PSD",
        }
    }
    ...
    pub fn ext(self) -> &'static str {
        match self {
            ...
            DialogMode::OpenPsd | DialogMode::ExportPsd => ".psd",
            _ => ".png",
        }
    }
    ...
    pub fn is_save(self) -> bool {
        matches!(
            self,
            DialogMode::SaveProject
                | DialogMode::ExportPng
                | DialogMode::ExportPngAlpha
                | DialogMode::ExportJpeg
                | DialogMode::ExportPsd
        )
    }
}
...
/// Папка настроек программы: %APPDATA%\Tpaint — там же лежат недавние файлы.
fn config_dir() -> Option<std::path::PathBuf> {
    let base = std::env::var("APPDATA").ok()?;
    let dir = std::path::PathBuf::from(base).join(crate::APP_NAME);
    std::fs::create_dir_all(&dir).ok()?;
    Some(dir)
}

/// Файл автосохранения: его не нужно выбирать, программа пишет его сама.
pub fn autosave_path() -> Option<std::path::PathBuf> {
    Some(config_dir()?.join("autosave.tpaint"))
}

/// Пометка «автосохранение уже показывали» — чтобы не восстанавливать одно
/// и то же по второму разу.
fn autosave_marker() -> Option<std::path::PathBuf> {
    Some(config_dir()?.join("autosave.done"))
}

/// Как часто программа пишет автосохранение, если документ менялся.
pub const AUTOSAVE_SECS: u64 = 60;

/// Переносит слои PSD в слои документа. Возвращает слои (в том же порядке, что
/// в файле: слой 0 — нижний) и число слоёв, у которых режим наложения пришлось
/// заменить: цветовые и градиентные режимы Photoshop мы не держим.
fn psd_layers(file: &crate::psd::PsdFile) -> (Vec<Layer>, usize) {
    use crate::psd::{blend_key, LayerKind};
    let (w, h) = (file.width, file.height);
    let mut out: Vec<Layer> = Vec::new();
    let mut replaced = 0;
    // Открытые папки снизу вверх: слои копятся, а имя приходит с шапкой.
    let mut open: Vec<Vec<usize>> = Vec::new();
    for l in file.layers.iter() {
        if l.kind == LayerKind::Bounding {
            open.push(Vec::new());
            continue;
        }
        let folder = matches!(l.kind, LayerKind::GroupOpen | LayerKind::GroupClosed);
        let name = if l.name.trim().is_empty() {
            if folder {
                format!("Группа {}", open.len())
            } else {
                format!("Слой {}", out.len() + 1)
            }
        } else {
            l.name.clone()
        };
        let mut layer = Layer::new(w, h, &name);
        if !folder {
            // Кадр слоя кладём на холст по его смещению, часть может уйти за край.
            for y in 0..l.height {
                let dy = l.top + y as i32;
                if dy < 0 || dy as usize >= h {
                    continue;
                }
                for x in 0..l.width {
                    let dx = l.left + x as i32;
                    if dx < 0 || dx as usize >= w {
                        continue;
                    }
                    layer.set(w, dx as usize, dy as usize, l.pixel(x, y));
                }
            }
        }
        layer.meta.visible = l.visible;
        layer.meta.opacity = l.opacity as f32 / 255.0;
        layer.meta.clipped = l.clipped;
        if l.blend_key != blend_key(l.blend) {
            replaced += 1;
        }
        layer.meta.blend = l.blend;
        layer.meta.group_open = l.kind != LayerKind::GroupClosed;
        if let Some(m) = &l.mask {
            let mut mask = vec![0u8; w * h];
            for y in 0..m.height {
                let dy = m.top + y as i32;
                if dy < 0 || dy as usize >= h {
                    continue;
                }
                for x in 0..m.width {
                    let dx = m.left + x as i32;
                    if dx < 0 || dx as usize >= w {
                        continue;
                    }
                    mask[dy as usize * w + dx as usize] = m.data[y * m.width + x];
                }
            }
            layer.mask = Some(mask);
            layer.mask_on = !m.disabled;
        }
        out.push(layer);
        let me = out.len() - 1;
        if folder {
            // Шапка папки: всем слоям, что копилось снизу, идёт её имя. Сама
            // шапка относится к своей папке, а если открыта внешняя — к ней.
            if let Some(waiting) = open.pop() {
                for i in waiting {
                    out[i].meta.group = Some(name.clone());
                }
            }
            out[me].meta.group = Some(name);
            if let Some(outer) = open.last_mut() {
                outer.push(me);
            }
        } else if let Some(waiting) = open.last_mut() {
            waiting.push(me);
        }
    }
    (out, replaced)
...
/// Кусок слоя размером с кисть, снятый в начале мазка: восстанавливающая
/// кисть впечатывает его под мазок, сдвигая вместе с курсором.
#[derive(Clone)]
pub struct HealPatch {
    pub data: Vec<u8>,
    pub w: usize,
    pub h: usize,
}

/// Один отпечаток кисти: обычный, «липкий» (цвет из слоя) или
/// восстанавливающий (впечатывание патча). Всё в одном месте, чтобы
/// симметрия и симметричные копии вели себя одинаково.
...
fn stamp_dab(
    ...
    let c = match (sticky, base) {
        (true, Some(b)) => raster::sample_shifted(b, w, h, p.0, p.1, hue, sat),
        _ => color,
    };
    raster::stamp_shape(layer, w, h, p.0, p.1, r, hard, c, op, shape);
}

/// Какое окно коррекции открыто: у всех один и тот же предпросмотр.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum CorrKind {
    Curves,
    Levels,
...
/// К чему притянулся курсор: показываем линию или точку на холсте.
#[derive(Clone, Copy, PartialEq, Debug)]
pub enum SnapHit {
    /// Вертикальная направляющая на позиции X холста.
    GuideV(f32),
    /// Горизонтальная направляющая на позиции Y холста.
    GuideH(f32),
    /// Точка сетки.
    Grid(f32, f32),
    /// Угол 15° от начала мазка.
    Angle,
}
...
pub struct App {
    ...
    /// Кусок слоя, который тянет за собой восстанавливающая кисть.
    pub heal_patch: Option<HealPatch>,
    ...
    /// Показывать навигатор — маленькую копию холста с рамкой вида.
    pub show_nav: bool,
    /// В навигаторе тянут рамку вида.
    pub nav_drag: bool,
    /// Направляющие: (позиция в пикселях холста, true — горизонтальная).
    pub guides: Vec<(f32, bool)>,
    /// Притягивать курсор к направляющим.
    pub snap_guides: bool,
    /// Притягивать курсор к сетке.
    pub snap_grid: bool,
    /// Притягивать угол мазка к шагу 15°.
    pub snap_angle: bool,
    ...
    /// Окно кривых с гистограммой.
    pub curve_dialog: bool,
    /// Канал, который правит кривая.
    pub curve_channel: raster::CurveChannel,
    /// Точки текущей кривой (0..=1).
    pub curve: raster::Curve,
    /// Пиксели слоя до правки кривой — из них каждый раз считается результат.
    pub curve_base: Option<Vec<u8>>,
    /// Гистограмма яркости активного слоя на момент открытия окна.
    pub curve_hist: [u32; 256],
    /// Какую точку кривой тянут мышью.
    pub curve_drag: Option<usize>,
    /// Окно уровней (входные и выходные точки, гамма).
    pub levels_dialog: bool,
    /// Какое окно коррекции открыто — общее состояние предпросмотра.
    pub corr: CorrKind,
    /// Входная чёрная точка уровней (0…255).
    pub lvl_in_black: f32,
    /// Входная белая точка уровней (0…255).
    pub lvl_in_white: f32,
    /// Гамма уровней (0.1…4).
    pub lvl_gamma: f32,
    /// Выходная чёрная точка (0…255).
    pub lvl_out_black: f32,
    /// Выходная белая точка (0…255).
    pub lvl_out_white: f32,
    /// Когда последний раз писали автосохранение.
    pub last_autosave: Instant,
    /// Окно «Тон и насыщенность».
    pub hsv_dialog: bool,
    /// Окно «Цветовой баланс».
    pub balance_dialog: bool,
    /// Сектор сдвига оттенка.
    pub hsv_channel: raster::HsvChannel,
    /// Сдвиг оттенка в градусах.
    pub hsv_hue: f32,
    /// Сдвиг насыщенности в процентах.
    pub hsv_sat: f32,
    /// Сдвиг светлоты в процентах.
    pub hsv_light: f32,
    /// Цветовой баланс: [тени; средние тона; света] × [R, G, B].
    pub balance: [[f32; 3]; 3],
...
impl App {
    pub fn new() -> Self {
        ...
        sel_mask_rev: 0,
            ...
            filter_dialog: false,
            curve_dialog: false,
            curve_channel: raster::CurveChannel::Rgb,
            curve: raster::Curve::linear(),
            curve_base: None,
            curve_hist: [0; 256],
            curve_drag: None,
            levels_dialog: false,
            corr: CorrKind::Curves,
            lvl_in_black: 0.0,
            lvl_in_white: 255.0,
            lvl_gamma: 1.0,
            lvl_out_black: 0.0,
            lvl_out_white: 255.0,
            last_autosave: Instant::now() - std::time::Duration::from_secs(AUTOSAVE_SECS),
            hsv_dialog: false,
            balance_dialog: false,
            hsv_channel: raster::HsvChannel::All,
            hsv_hue: 0.0,
            hsv_sat: 0.0,
            hsv_light: 0.0,
            balance: [[0.0; 3]; 3],
    ...
    /// Показывает в виде центре экрана точку холста — этим пользуется навигатор.
    pub fn center_on(&mut self, x: f32, y: f32) {
        let r = self.canvas_rect;
        self.pan.0 = (r[2] - r[0]) * 0.5 - x * self.zoom;
        self.pan.1 = (r[3] - r[1]) * 0.5 - y * self.zoom;
    }
    ...
    /// Снимает кусок слоя вокруг точки: столько, сколько покрывает кисть.
    fn grab_heal_patch(&self, p: (f32, f32)) -> Option<HealPatch> {
        let (w, h) = (self.doc.width, self.doc.height);
        let r = (self.params.size * 0.5).max(1.0) as i32 + 1;
        let cx = p.0 as i32;
        let cy = p.1 as i32;
        let pw = (r * 2) as usize;
        let ph = (r * 2) as usize;
        // Патч может вылезти за холст — тогда берём только то, что внутри.
        let mut data = vec![0u8; pw * ph * 4];
        let src = &self.doc.active_layer().pixels;
        for y in 0..ph {
            for x in 0..pw {
                let sx = cx - (pw as i32) / 2 + x as i32;
                let sy = cy - (ph as i32) / 2 + y as i32;
    ...
    pub fn begin_stroke(&mut self, p: (f32, f32), secondary: bool) {
        ...
        // Восстанавливающая кисть снимает кусок слоя вокруг начала мазка —
        // дальше он впечатывается под кисть.
        self.heal_patch = if self.params.heal && self.tool == Tool::Brush {
            self.grab_heal_patch(p)
        } else {
            None
        };
        self.drawing = true;
        ...
        if self.tool.is_freehand() {
            ...
            let layer = self.doc.active_layer_mut();
            // Первая точка мазка: сразу ставим отпечаток и его отражения.
    ...
    pub fn commit_text(&mut self) {
        ...
        let vertical = self.params.vertical_text;
        let layer = self.doc.active_layer_mut();
        if vertical {
            // Столбик: точка клика — верх колонки, буквы идут вниз.
            crate::text::draw_text_vertical(
                layer, w, h, &t.buf, t.x, t.y, size, color, opacity, bold,
            );
        } else {
            crate::text::draw_text(layer, w, h, &t.buf, t.x, t.y, size, color, opacity, bold);
        }
        let after = self.doc.layers[self.doc.active].pixels.clone();
        if before != after {
    ...
    pub fn move_select(&mut self, p: (f32, f32)) {
        // Рамку тоже тянем с привязкой — так ровнее попасть в линию.
        let p = self.apply_snap(p, None);
        match self.sel_drag {
            SelDrag::Marquee => {
    ...
    pub fn snap_preview(&self, p: (f32, f32), from: Option<(f32, f32)>) -> Option<((f32, f32), SnapHit)> {
        ...
        let tol = 6.0 / self.zoom.max(0.05);
        let mut best: Option<(f32, SnapHit)> = None;
        let offer = |d: f32, hit: SnapHit, best: &mut Option<(f32, SnapHit)>| {
            if d <= tol && best.map(|(bd, _)| d < bd).unwrap_or(true) {
                *best = Some((d, hit));
            }
        };
        // Направляющие: тянут к себе по своей оси.
        if self.snap_guides {
            for (pos, horiz) in &self.guides {
        ...
        // Сетка — только когда она включена и видна.
        if self.snap_grid && self.show_grid && self.grid_size > 0.0 {
            let gx = (p.0 / self.grid_size).round() * self.grid_size;
            let gy = (p.1 / self.grid_size).round() * self.grid_size;
            if (gx - p.0).abs() <= tol {
                offer((gx - p.0).abs(), SnapHit::Grid(gx, gy), &mut best);
            }
            if (gy - p.1).abs() <= tol {
                offer((gy - p.1).abs(), SnapHit::Grid(gx, gy), &mut best);
            }
        }
        let mut out = p;
        // Угол: держим мазок на шаге 15° от начала.
        if let Some(f) = angle_from {
            let (dx, dy) = (p.0 - f.0, p.1 - f.1);
            let len = (dx * dx + dy * dy).sqrt();
            if len > tol * 2.0 {
                let step = std::f32::consts::TAU / 24.0;
                let a = (dy.atan2(dx) / step).round() * step;
                let (nx, ny) = (f.0 + a.cos() * len, f.1 + a.sin() * len);
                if (nx - p.0).abs() <= tol && (ny - p.1).abs() <= tol {
                    best = Some((0.0, SnapHit::Angle));
                    out = (nx, ny);
                }
            }
        }
        let (_, hit) = best?;
        let q = match hit {
            SnapHit::GuideV(x) => (x, out.1),
            SnapHit::GuideH(y) => (out.0, y),
            SnapHit::Grid(x, y) => (x, y),
            SnapHit::Angle => out,
        };
        Some((q, hit))
    }

    /// Притягивает точку к направляющим, сетке и (при включённом шаге)
    /// к углу 15°.
    pub fn apply_snap(&mut self, p: (f32, f32), from: Option<(f32, f32)>) -> (f32, f32) {
        self.snap_preview(p, from).map(|(q, _)| q).unwrap_or(p)
    }
    ...
    pub fn toggle_fx_glow(&mut self) {
        let i = self.doc.active;
    ...
    // --- кривые (тоновая коррекция) ---

    /// Открывает окно кривых. Пока окно открыто, слой показывает результат
    /// правки прямо на холсте, а исходные пиксели хранятся в `curve_base`:
    /// так «Отмена» возвращает слой целиком, а история получает один шаг.
    pub fn open_curves(&mut self) {
        if self.doc.active_layer().meta.locked {
            self.notify("Слой заблокирован");
            return;
        }
        self.end_stroke();
        self.curve = raster::Curve::linear();
        self.curve_hist = raster::histogram(self.doc.active_layer());
        self.curve_base = Some(self.doc.active_layer().pixels.clone());
        self.corr = CorrKind::Curves;
        self.curve_dialog = true;
    }

    /// Пересчитывает слой по текущей кривой (предпросмотр без истории).
    pub fn update_curve(&mut self) {
        if matches!(self.corr, CorrKind::HueSat | CorrKind::Balance) {
            self.update_color();
            return;
        }
        ...
        raster::apply_curve(&mut self.doc.layers[i], self.curve_channel, &lut);
        self.doc.touch();
        self.dirty = true;
    }

    /// Таблица коррекции: кривая или уровни — что открыто в окне.
    pub fn corr_lut(&self) -> [u8; 256] {
        match self.corr {
            CorrKind::Curves => self.curve.lut(),
            CorrKind::Levels => raster::levels_lut(
                self.lvl_in_black,
                self.lvl_in_white,
                self.lvl_gamma,
                self.lvl_out_black,
            ...
            // У этих двух окон таблицы нет: правка идёт прямо по пикселям.
            CorrKind::HueSat | CorrKind::Balance => self.curve.lut(),
        }
    }

    /// Открывает окно «Тон и насыщенность» на базу активного слоя.
    pub fn open_hsv(&mut self) {
        if self.doc.active_layer().meta.locked {
            self.notify("Слой заблокирован");
            return;
        }
        ...
        self.curve_hist = raster::histogram(self.doc.active_layer());
        self.curve_base = Some(self.doc.active_layer().pixels.clone());
        self.corr = CorrKind::HueSat;
        self.hsv_dialog = true;
    }
    ...
    pub fn update_color(&mut self) {
        ...
        match corr {
            CorrKind::HueSat => raster::hsv_adjust(layer, ch, hue, sat, light),
            CorrKind::Balance => raster::color_balance(layer, bal[0], bal[1], bal[2]),
            // Кривые и уровни идут через таблицу значений.
            _ => raster::apply_curve(layer, self.curve_channel, &lut),
        }
        self.doc.touch();
        self.dirty = true;
    }

    /// Открывает окно цветового баланса.
    pub fn open_balance(&mut self) {
        if self.doc.active_layer().meta.locked {
            self.notify("Слой заблокирован");
            return;
        }
        ...
        self.curve_hist = raster::histogram(self.doc.active_layer());
        self.curve_base = Some(self.doc.active_layer().pixels.clone());
        self.corr = CorrKind::Balance;
        self.balance_dialog = true;
    }

    /// Возвращает параметры тона и насыщенности к нулю.
    pub fn hsv_reset(&mut self) {
        self.hsv_hue = 0.0;
        self.hsv_sat = 0.0;
        self.hsv_light = 0.0;
        self.update_color();
    ...
    /// Возвращает цветовой баланс к нулю.
    pub fn balance_reset(&mut self) {
        self.balance = [[0.0; 3]; 3];
        self.update_color();
    }

    /// Открывает окно уровней на базу активного слоя.
    pub fn open_levels(&mut self) {
        if self.doc.active_layer().meta.locked {
            self.notify("Слой заблокирован");
            return;
        }
        ...
        self.curve_hist = raster::histogram(self.doc.active_layer());
        self.curve_base = Some(self.doc.active_layer().pixels.clone());
        self.corr = CorrKind::Levels;
        self.levels_dialog = true;
    }

    /// Возвращает уровни к исходным значениям.
    pub fn levels_reset(&mut self) {
        self.lvl_in_black = 0.0;
        self.lvl_in_white = 255.0;
        self.lvl_gamma = 1.0;
        self.lvl_out_black = 0.0;
        self.lvl_out_white = 255.0;
        self.update_curve();
    }

    /// Закрывает окно коррекции: `save` = оставить правку и записать шаг.
    pub fn close_corr(&mut self, save: bool) {
        self.curve_dialog = false;
        ...
        if !save {
            ...
            self.doc.touch();
            self.dirty = true;
            self.notify("Правка отменена");
            return;
        }
        let after = self.doc.layers[i].pixels.clone();
        if base != after {
            let name = match self.corr {
                CorrKind::Curves => "Кривые",
                CorrKind::Levels => "Уровни",
    ...
    /// Переносит точку кривой в координатах окна и обновляет предпросмотр.
    pub fn curve_point_to(&mut self, x: f32, y: f32) -> usize {
        let i = self.curve.set_point(x, y, 0.02);
        self.update_curve();
        i
    }

    /// Двигает уже существующую точку, не добавляя новых: соседей она не
    /// перепрыгивает, крайние — только по вертикали.
    pub fn curve_move_point(&mut self, i: usize, x: f32, y: f32) {
        let last = self.curve.pts.len().saturating_sub(1);
        if i > last {
            return;
        }
        if i == 0 || i == last {
            self.curve.pts[i].1 = y.clamp(0.0, 1.0);
        } else {
            let lo = self.curve.pts[i - 1].0 + 0.01;
            let hi = self.curve.pts[i + 1].0 - 0.01;
            self.curve.pts[i] = (x.clamp(lo.min(hi), hi.max(lo)), y.clamp(0.0, 1.0));
        }
        self.update_curve();
    }

    /// Убирает внутреннюю точку кривой.
    pub fn curve_point_remove(&mut self, i: usize) {
        self.curve.remove_point(i);
        self.update_curve();
    }

    /// Возвращает кривую к прямой и пересчитывает слой.
    pub fn curve_reset(&mut self) {
        self.curve = raster::Curve::linear();
        self.update_curve();
    }

    /// Закрывает окно кривых: `save` = оставить правку и записать шаг истории.
    pub fn close_curves(&mut self, save: bool) {
        self.close_corr(save);
    }

    // --- автосохранение ---

    /// Пишет проект в служебный файл, если есть несохранённые изменения.
    /// Возвращает true, когда файл действительно записан.
    ...
    /// Пора ли сохранять: прошло достаточно времени с прошлого раза.
    pub fn autosave_due(&self) -> bool {
        self.dirty && self.last_autosave.elapsed().as_secs() >= AUTOSAVE_SECS
    }
    ...
    pub fn restore_autosave(&mut self) -> Result<(), String> {
        ...
        self.notify("Восстановлено автосохранение");
        Ok(())
    }

    /// Убирает автосохранение — он больше не нужен, документ сохранён руками.
    pub fn drop_autosave() {
    ...
    pub fn save_project(&mut self, path: &str) -> Result<(), String> {
        ...
        self.dirty = false;
        // Документ сохранён руками — служебная копия больше не нужна.
        Self::drop_autosave();
    ...
    /// Открывает PSD: слои файла становятся слоями документа вместе с
    /// масками, папками, непрозрачностью и режимами наложения. Если слоёв в
    /// файле нет, берётся итоговая картинка документа.
    pub fn open_psd(&mut self, path: &str) -> Result<(), String> {
        self.end_stroke();
        let bytes = std::fs::read(path).map_err(|e| e.to_string())?;
        let file = crate::psd::read_file(&bytes)?;
        if file.width == 0 || file.height == 0 {
            return Err("пустой PSD".to_string());
        }
        let (w, h) = (file.width, file.height);
        let (mut layers, replaced) = psd_layers(&file);
        if layers.is_empty() {
            // Слоёв в файле нет — открываем итоговую картинку.
            let mut l = Layer::new(w, h, "PSD");
            l.pixels.copy_from_slice(&file.merged.pixels);
            layers.push(l);
        }
        ...
        doc.height = h;
        doc.layers = layers;
        // Активен верхний слой — так список слоёв не уезжает вниз.
        doc.active = doc.layers.len().saturating_sub(1);
        doc.touch();
        self.doc = doc;
        self.history = History::new(40);
        // Путь не запоминаем: поверх PSD имеет смысл только сохранение проекта.
        self.path = None;
        self.dirty = true;
        self.fit_pending = true;
        self.selection = None;
        ...
        if replaced > 0 {
            self.notify(&format!(
                "Открыт PSD {}×{}, слоёв: {} (режимы наложения заменены: {})",
                w, h, n, replaced
            ));
        } else {
            self.notify(&format!("Открыт PSD {}×{}, слоёв: {}", w, h, n));
        }
        Ok(())
    }

    /// Экспорт в PSD: слои, маски, папки, непрозрачность, видимость и режимы
    /// наложения плюс итоговая картинка документа.
    pub fn export_psd(&mut self, path: &str) -> Result<(), String> {
        self.doc.ensure_composite();
        let (w, h) = (self.doc.width, self.doc.height);
        let out: Vec<crate::psd::OutLayer> = self
            .doc
            .layers
            .iter()
            .enumerate()
            .map(|(i, l)| crate::psd::OutLayer {
                name: &l.meta.name,
                rgba: &l.pixels,
                mask: l.mask.as_deref(),
                opacity: (l.meta.opacity * 255.0).round().clamp(0.0, 255.0) as u8,
                visible: l.meta.visible,
                blend: l.meta.blend,
                group: l.meta.group.as_deref(),
                // Шапка папки — самый верхний (последний) слой своей группы.
                group_header: l.meta.group.is_some()
                    && (i + 1 == self.doc.layers.len()
                        || self.doc.layers[i + 1].meta.group != l.meta.group),
                group_open: l.meta.group_open,
            })
            .collect();
...
mod tests {
    ...
    fn project_keeps_layer_effects_and_tag() {
        ...
        assert_eq!(m.fx.glow_blur, 12.0);
        assert_eq!(m.fx.glow_color, [40, 50, 60, 255]);
    ...
    #[test]
    fn glow_draws_halo_around_layer() {
        ...
        app.set_layer_meta(0, |m| {
            m.fx.glow_blur = 6.0;
            m.fx.glow_color = [255, 80, 0, 255];
        });
        app.doc.touch();
        app.doc.ensure_composite();
        assert_eq!(app.doc.layers[0].pixels, before, "свечение неразрушающее");
        let c = &app.doc.composite;
        let at = |x: usize, y: usize| c[(y * 60 + x) * 4 + 3];
        assert!(at(15, 30) > 0, "ореол слева от квадрата");
    ...
    #[test]
    fn psd_file_is_written_for_manual_check() {
        // Готовый PSD в %TEMP%, чтобы открыть его из командной строки:
        // tpaint.exe tpaint_app_<pid>_образец.psd
        let (w, h) = (600, 400);
        ...
        for y in 0..h {
            for x in 0..w {
                ...
                px[i] = (t * 235.0) as u8;
                px[i + 1] = (u * 200.0) as u8;
                px[i + 2] = ((t + u) * 0.5 * 255.0) as u8;
                px[i + 3] = 255;
            }
        }
        // Круг в центре — видно, что альфа и геометрия на месте.
        for y in 0..h {
            for x in 0..w {
                ...
                if dx * dx + dy * dy < 90.0 * 90.0 {
                    let i = (y * w + x) * 4;
                    px[i..i + 4].copy_from_slice(&[255, 240, 90, 255]);
                }
            }
        }
    ...
    #[test]
    fn curve_dialog_previews_and_commits_one_history_step() {
        let mut app = app_with_blank(20, 20);
        // Полоса серого — после кривой станет контрастнее.
        for x in 0..20 {
            let v = (x * 12) as u8;
            for y in 0..20 {
                app.doc.active_layer_mut().set(20, x, y, [v, v, v, 255]);
            }
        }
        let before = app.doc.layers[0].pixels.clone();
        app.open_curves();
        assert!(app.curve_dialog, "окно открыто");
        assert!(app.curve_base.is_some(), "исходные пиксели сохранены");
        // Тянем точку вверх: тёмные значения должны стать темнее.
        let i = app.curve_point_to(0.25, 0.05);
        assert!(i > 0 && i + 1 < app.curve.pts.len());
        ...
        // Отмена отдаёт исходный слой.
        app.undo();
        assert_eq!(app.doc.layers[0].pixels, before, "Ctrl+Z вернул слой");
    }

    #[test]
    fn curve_dialog_cancel_restores_layer() {
        let mut app = app_with_blank(8, 8);
        app.doc.active_layer_mut().fill([90, 90, 90, 255]);
        let before = app.doc.layers[0].pixels.clone();
        app.open_curves();
    ...
    #[test]
    fn curve_reset_returns_straight_line() {
        let mut app = app_with_blank(8, 8);
        app.open_curves();
    ...
    #[test]
    fn curve_point_drag_does_not_multiply_points() {
        let mut app = app_with_blank(8, 8);
        app.open_curves();
        let i = app.curve_point_to(0.5, 0.5);
        assert_eq!(app.curve.pts.len(), 3);
        // Протяжка — это движение той же точки, а не добавление новых.
        for k in 1..8 {
            app.curve_move_point(i, 0.5 + k as f32 * 0.03, 0.4);
        }
        assert_eq!(app.curve.pts.len(), 3, "точек по-прежнему три");
        // Точка не перепрыгивает через соседей.
        app.curve_move_point(i, 2.0, 0.5);
        assert!(app.curve.pts[i].0 < 1.0, "точка осталась между крайними");
        // Крайние точки двигаются только по вертикали.
        app.curve_move_point(0, 0.9, 0.3);
        assert_eq!(app.curve.pts[0], (0.0, 0.3), "левая точка не уехала вправо");
    }

    #[test]
    fn levels_dialog_previews_and_commits_one_step() {
        let mut app = app_with_blank(16, 16);
        for y in 0..16 {
            for x in 0..16 {
                let v = (x * 16) as u8;
                app.doc.active_layer_mut().set(16, x, y, [v, v, v, 255]);
            }
        }
        let before = app.doc.layers[0].pixels.clone();
        app.open_levels();
    ...
    #[test]
    fn levels_reset_returns_neutral_values() {
        let mut app = app_with_blank(8, 8);
        app.open_levels();
        app.lvl_gamma = 3.0;
        app.lvl_out_white = 200.0;
        app.levels_reset();
        assert_eq!(app.lvl_gamma, 1.0);
        assert_eq!(app.lvl_in_black, 0.0);
        assert_eq!(app.lvl_out_white, 255.0);
    }

    #[test]
    fn autosave_writes_project_and_is_dropped_on_save() {
        let Some(path) = autosave_path() else {
            return; // нет %APPDATA% — проверять нечего
        };
        App::drop_autosave();
        let mut app = app_with_blank(10, 10);
        assert!(!app.autosave_now(), "чистый документ не сохраняем");
        app.doc.active_layer_mut().fill([7, 8, 9, 255]);
        app.dirty = true;
        assert!(app.autosave_now(), "файл автосохранения записан");
        assert!(path.exists(), "файл на диске: {}", path.display());
        assert!(App::autosave_pending(), "есть что восстанавливать");
        ...
        app.dirty = true;
        app.autosave_now();
        let out = temp_file("сохранён.tpaint");
        app.save_project(&out).expect("сохранили проект");
        let _ = std::fs::remove_file(&out);
        assert!(!path.exists(), "автосохранение удалено");
    }

    #[test]
    fn sticky_brush_paints_with_color_under_the_cursor() {
        let mut app = app_with_blank(40, 40);
        ...
        app.tool = Tool::Brush;
        app.params.size = 8.0;
        app.params.sticky = true;
        app.primary = [0, 0, 0, 255];
        // Мазок по границе цветов: слева красный, справа синий.
        app.begin_stroke((5.0, 30.0), false);
        app.move_stroke((15.0, 30.0), false);
        app.end_stroke();
    ...
    #[test]
    fn healing_brush_copies_texture_into_the_stroke() {
        let mut app = app_with_blank(40, 40);
        ...
        app.tool = Tool::Brush;
        app.params.size = 16.0;
        app.params.hardness = 1.0;
        app.params.heal = true;
        app.params.heal_strength = 1.0;
        app.params.spacing = 0.05;
        // Замазываем зелёным прямоугольником, затем «лечим» мазком.
        for y in 26..34 {
            for x in 4..20 {
                app.doc.active_layer_mut().set(40, x, y, [0, 220, 0, 255]);
            }
        }
        let before = app.doc.active_layer().get(40, 10, 30);
        assert_eq!(before, [0, 220, 0, 255], "полоса закрашена");
        // Патч берётся в начале мазка (вне закрашенной области) и едет с кистью.
        app.begin_stroke((30.0, 30.0), false);
        app.move_stroke((18.0, 30.0), false);
        app.move_stroke((10.0, 30.0), false);
        app.end_stroke();
        let after = app.doc.active_layer().get(40, 10, 30);
        assert_ne!(after, before, "восстанавливающая кисть впечатала текстуру");
        assert!(after[1] < 200, "зелёный ушёл: {:?}", after);
    }

    #[test]
    fn hue_saturation_dialog_previews_and_commits() {
        let mut app = app_with_blank(8, 8);
        for y in 0..8 {
            for x in 0..8 {
                app.doc.active_layer_mut().set(8, x, y, [200, 40, 60, 255]);
            }
        }
        let before = app.doc.layers[0].pixels.clone();
        app.open_hsv();
        app.hsv_channel = raster::HsvChannel::Red;
        app.hsv_hue = 120.0;
        app.update_color();
        let after = app.doc.active_layer().get(8, 4, 4);
        assert!(after[1] > 120, "красный ушёл в зелёный: {:?}", after);
        app.close_corr(true);
        assert!(app.history.can_undo(), "правка в истории");
        app.undo();
        assert_eq!(app.doc.layers[0].pixels, before, "Ctrl+Z вернул слой");
    }

    #[test]
    fn balance_dialog_shifts_tones_and_commits() {
        let mut app = app_with_blank(8, 8);
        for y in 0..8 {
            for x in 0..8 {
                app.doc.active_layer_mut().set(8, x, y, [30, 30, 30, 255]);
            }
        }
        let before = app.doc.layers[0].pixels.clone();
        app.open_balance();
    ...
    #[test]
    fn snap_pulls_cursor_to_guide() {
        let mut app = App::new();
        app.zoom = 1.0;
        app.snap_guides = true;
        app.guides.push((100.0, false));
        ...
        assert!(app.snap_preview((130.0, 40.0), None).is_none());
        // Выключенная привязка ничего не делает.
        app.snap_guides = false;
        assert_eq!(app.apply_snap((103.0, 40.0), None), (103.0, 40.0));
    }

    #[test]
    fn snap_to_grid_works_only_when_grid_is_shown() {
        let mut app = App::new();
        app.zoom = 1.0;
        app.snap_grid = true;
        app.show_grid = true;
    ...
    #[test]
    fn snap_angle_keeps_stroke_on_fifteen_degrees() {
        let mut app = App::new();
        app.zoom = 1.0;
        app.snap_angle = true;
    ...
    fn demo_project_is_written_for_manual_check() {
        ...
        app.params.symmetry = Symmetry::Vertical;
        ...
        app.params.symmetry = Symmetry::Off;
        ...
        app.show_grid = true;
        app.grid_size = 64.0;
        // Вертикальный текст отдельным слоем — виден столбик букв.
        app.add_layer();
        app.doc.layers[5].meta.name = "Вертикаль".to_string();
        crate::text::draw_text_vertical(
            app.doc.active_layer_mut(),
            800,
            500,
        ...
        let vis: Vec<bool> = (0..opened.doc.layers.len())
            .map(|i| opened.layer_visible_in_panel(i))
            .collect();
        ...
        assert!(
            opened.doc.layers.iter().any(|l| l.mask.is_some()),
            "маска на месте: {:?}",
            names
        );
    }
    ...
    #[test]
    fn vertical_text_lays_glyphs_in_a_column() {
        let mut app = app_with_blank(200, 300);
        app.primary = [0, 0, 0, 255];
        app.tool = Tool::Text;
        app.params.size = 40.0;
        app.params.vertical_text = true;
        app.begin_text((100.0, 20.0));
        for c in "АБВ".chars() {
            app.text_input(c);
        }
        app.commit_text();
        // Столбик: три буквы занимают три строки, поэтому высота больше ширины.
        let (mut top, mut bottom, mut left, mut right) = (usize::MAX, 0usize, usize::MAX, 0usize);
        for y in 0..300 {
            for x in 0..200 {
                if app.doc.active_layer().get(200, x, y)[3] > 0 {
                    top = top.min(y);
                    bottom = bottom.max(y);
                    left = left.min(x);
                    right = right.max(x);
    ...
    #[test]
    fn debug_vertical_art_tmp() {
        let mut app = app_with_blank(80, 60);
        app.primary = [0, 0, 0, 255];
        app.tool = Tool::Text;
        app.params.size = 32.0;
        app.params.vertical_text = true;
        app.begin_text((40.0, 4.0));
        for c in "АБ".chars() {
            app.text_input(c);
        }
        app.commit_text();
    ...
    #[test]
    fn center_on_puts_point_in_the_middle_of_the_view() {
        let mut app = App::new();
        app.canvas_rect = [46.0, 26.0, 1146.0, 876.0];
        app.zoom = 1.5;
        app.pan = (0.0, 0.0);
        app.center_on(400.0, 250.0);
    ...
    #[test]
    fn export_psd_and_open_it_back() {
        let mut app = app_with_blank(12, 9);
        app.doc.active_layer_mut().fill([10, 120, 240, 255]);
        // Часть холста оставляем прозрачной — альфа тоже должна вернуться.
        for y in 0..4 {
            for x in 0..5 {
                app.doc.active_layer_mut().set(12, x, y, [0, 0, 0, 0]);
            }
        }
        let path = temp_file("обмен.psd");
        app.export_psd(&path).expect("экспорт");
        assert!(std::path::Path::new(&path).exists(), "файл PSD записан");
        let mut back = App::new();
        back.open_psd(&path).expect("открыли свой PSD");
        let _ = std::fs::remove_file(&path);
    ...
    #[test]
    fn psd_export_keeps_layers_masks_and_groups() {
        let mut app = app_with_blank(10, 8);
        app.doc.active_layer_mut().fill([20, 40, 60, 255]);
        // Второй слой поверх, с маской и режимом наложения.
        app.add_layer();
        app.doc.active_layer_mut().fill([200, 30, 90, 128]);
        app.doc.layers[1].meta.blend = crate::doc::BlendMode::Multiply;
        let mut mask = vec![255u8; 80];
        for y in 0..3 {
            for x in 0..10 {
                mask[y * 10 + x] = 30;
            }
        }
        app.doc.layers[1].mask = Some(mask);
        // Третий слой кладём в папку: второй и третий станут содержимым.
        app.add_layer();
        app.doc.active_layer_mut().fill([250, 250, 250, 255]);
        app.doc.active = 1;
        app.make_group();
        app.doc.layers[2].meta.name = "Облако".to_string();
        let path = temp_file("слои.psd");
        app.export_psd(&path).expect("экспорт со слоями");
        let mut back = App::new();
        back.open_psd(&path).expect("открыли свой PSD со слоями");
        let _ = std::fs::remove_file(&path);
        ...
        assert_eq!(
            back.doc.layers[1].meta.blend,
            crate::doc::BlendMode::Multiply,
            "режим наложения"
        );
        let m = back.doc.layers[1].mask.as_ref().expect("маска переехала");
        assert_eq!(m.len(), 80, "маска на весь холст");
        assert_eq!(m[0], 30, "верх маски закрыт");
        assert_eq!(m[79], 255, "низ маски открыт");
    }

    #[test]
    fn psd_import_reads_layer_settings_from_file() {
        use crate::psd::OutLayer;
        // Собираем файл писателем psd, но проверяем именно импорт в документ.
        let (w, h) = (5, 4);
        let mut top = vec![0u8; w * h * 4];
        let mut bottom = vec![0u8; w * h * 4];
        for y in 0..h {
            for x in 0..w {
                let i = (y * w + x) * 4;
                top[i..i + 4].copy_from_slice(&[255, 0, 0, 255]);
                bottom[i..i + 4].copy_from_slice(&[0, 0, 255, 160]);
            }
        }
        let file = crate::psd::write_layers(
            w,
            h,
            &bottom,
            &[
                OutLayer {
                    ...
                    opacity: 255,
                    visible: true,
                    blend: crate::doc::BlendMode::Normal,
                    group: None,
                    group_header: false,
                    group_open: true,
                },
                OutLayer {
                    ...
                    opacity: 200,
                    visible: false,
                    blend: crate::doc::BlendMode::Screen,
                    group: None,
                    group_header: false,
                    group_open: true,
                },
        ...
        let parsed = crate::psd::read_file(&file).expect("разбор файла");
        let (layers, replaced) = psd_layers(&parsed);
        assert_eq!(replaced, 0, "оба режима мы умеем");
        assert_eq!(layers.len(), 2, "слоя на месте");
        // Порядок документа совпадает с порядком в файле: слой 0 — нижний.
        assert_eq!(layers[0].meta.name, "Земля");
        assert_eq!(layers[1].meta.name, "Небо");
    ...
    #[test]
    fn psd_import_is_reported_with_size() {
        let mut app = app_with_blank(4, 4);
        let px: Vec<u8> = (0..8 * 6 * 4).map(|i| (i % 251) as u8).collect();
        let path = temp_file("не_то.psd");
        std::fs::write(&path, crate::psd::write(8, 6, &px)).expect("записали PSD");
        app.open_psd(&path).expect("открылся PSD");
        let _ = std::fs::remove_file(&path);
        assert_eq!((app.doc.width, app.doc.height), (8, 6));
        assert_eq!(app.doc.layers[0].pixels, px);
    }

    #[test]
    fn open_project_accepts_psd_path() {
        let mut app = app_with_blank(4, 4);
    ...
    #[test]
    fn broken_psd_gives_readable_error() {
        let mut app = app_with_blank(4, 4);
        let path = temp_file("мусор.psd");
        std::fs::write(&path, "это вообще не PSD".as_bytes()).expect("записали мусор");
        let err = app.open_psd(&path).expect_err("мусор не открывается");
        let _ = std::fs::remove_file(&path);
        assert!(err.contains("PSD"), "понятное сообщение: {}", err);
    }

    /// Число закрашенных пикселей от одного отпечатка кисти с симметрией.
    /// Точка (20, 40) выбрана не на оси симметрии, иначе отражения слились бы.
    fn dab_count(sym: Symmetry, sides: f32) -> usize {
        let mut app = app_with_blank(120, 120);
        app.params.size = 8.0;
        app.params.symmetry = sym;
        app.params.sym_sides = sides;
        app.tool = Tool::Brush;
        app.begin_stroke((20.0, 40.0), false);
        app.end_stroke();
        let l = app.doc.active_layer();
        (0..120 * 120).filter(|i| l.pixels[i * 4 + 3] > 0).count()
    }

    /// Сколько отдельных отпечатков оставил мазок: точки, разнесённые шире
    /// диаметра кисти, считаются разными пятнами.
    fn dab_clusters(sym: Symmetry, sides: f32) -> usize {
        let mut app = app_with_blank(120, 120);
        app.params.size = 8.0;
        app.params.symmetry = sym;
        app.params.sym_sides = sides;
        ...
        app.end_stroke();
        let l = app.doc.active_layer();
        let mut marks: Vec<(i32, i32)> = Vec::new();
        for y in 0..120i32 {
            for x in 0..120i32 {
    ...
    #[test]
    fn symmetry_multiplies_stroke_copies() {
    ...
    #[test]
    fn vertical_symmetry_mirrors_stroke_across_middle() {
        ...
        app.end_stroke();
        let painted = |x: usize, y: usize| app.doc.active_layer().get(100, x, y)[3] > 0;
        assert!(painted(25, 30), "исходный мазок на месте");
        assert!(painted(75, 30), "отражение у правого края");
        assert!(!painted(50, 30), "середина холста не закрашена");
    }

    #[test]
    fn center_symmetry_rotates_stroke_around_middle() {
        let mut app = app_with_blank(100, 100);
        app.params.size = 10.0;
        app.params.symmetry = Symmetry::Center;
        app.tool = Tool::Brush;
        app.begin_stroke((20.0, 20.0), false);
        app.move_stroke((30.0, 25.0), false);
        app.end_stroke();
        let painted = |x: usize, y: usize| app.doc.active_layer().get(100, x, y)[3] > 0;
        assert!(painted(25, 22), "исходный мазок в левом верхнем углу");
        assert!(painted(75, 78), "отражение в правом нижнем");
    }

    #[test]
    fn radial_symmetry_keeps_copies_at_same_radius() {
        let mut app = app_with_blank(120, 120);
        app.params.size = 8.0;
        app.params.symmetry = Symmetry::Radial;
        app.params.sym_sides = 6.0;
        app.tool = Tool::Brush;
        app.begin_stroke((60.0, 20.0), false);
        app.end_stroke();
        // Отпечатки расходятся лучами, но все на равном расстоянии от центра.
        let layer = app.doc.active_layer();
        let mut near = 0.0f32;
        let mut far = 0.0f32;
        for y in 0..120 {
            for x in 0..120 {
                if layer.get(120, x, y)[3] > 0 {
