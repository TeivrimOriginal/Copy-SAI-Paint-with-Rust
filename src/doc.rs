...
impl BlendMode {
    ...
    pub fn name(self) -> &'static str {
        match self {
            ...
            BlendMode::ColorDodge => "Цвет. осветление",
            BlendMode::ColorBurn => "Цвет. затемнение",
        }
    }
    ...
    fn mix(self, src: [f32; 3], dst: [f32; 3]) -> [f32; 3] {
        match self {
            ...
            // «Цветовые» осветление и затемнение зависят от подложки, поэтому
            // считаем их делением — как в Photoshop.
            BlendMode::ColorDodge => [
                dodge(src[0], dst[0]),
                dodge(src[1], dst[1]),
                dodge(src[2], dst[2]),
...
/// Осветление цветом: src вытягивает в свет тёмные участки подложки.
fn dodge(s: f32, d: f32) -> f32 {
    if d <= 0.0 {
        0.0
    } else if d >= 255.0 {
        255.0
    } else {
        (s * 255.0 / (255.0 - d)).min(255.0)
    }
}

/// Затемнение цветом: src съедает светлые участки подложки.
fn burn(s: f32, d: f32) -> f32 {
    if d <= 0.0 {
        0.0
    } else if d >= 255.0 {
        255.0
    } else {
        255.0 - ((255.0 - s) * 255.0 / d).min(255.0)
    }
}
...
pub struct LayerMeta {
    ...
    pub group_open: bool,
    /// Слой прижат к нижнему: видно только там, где нижний непрозрачен.
    /// Так делают слой-маску поверх слоя с цветом в Clip Studio.
    pub clipped: bool,
    /// Цветная метка слоя: 0 — нет метки, 1..=6 — один из шести цветов.
    pub tag: u8,
    /// Неразрушающие эффекты слоя (тени и обводка), как в Clip Studio.
    pub fx: LayerFx,
}

/// Палитра меток слоёв: индекс → цвет полоски слева.
pub const TAG_COLORS: [[f32; 4]; 6] = [
    [0.85, 0.25, 0.25, 1.0],
    [0.90, 0.55, 0.15, 1.0],
    [0.85, 0.78, 0.20, 1.0],
    [0.30, 0.75, 0.35, 1.0],
...
/// Тень и обводка слоя. Оба неразрушающие: пиксели слоя не меняются,
/// эффекты считаются при наложении.
#[derive(Clone, PartialEq, Debug)]
pub struct LayerFx {
    pub shadow: bool,
    pub shadow_dx: f32,
...
impl LayerFx {
    /// Есть ли хоть один включённый эффект.
    pub fn any(&self) -> bool {
        self.shadow || self.outline || self.glow
    }
}
...
impl Document {
    ...
    pub fn recompose(&mut self) {
        ...
        // Композит мог остаться пустым — документ собран из файла (PSD, проект).
        ...
        // Слой-маска: прижатый слой виден только там, где нижний непрозрачен.
        // Одна база обслуживает всю группу прижатых слоёв над ней.
        let mut base_alpha: Option<Vec<u8>> = None;
        for layer in self.layers.iter() {
            ...
            // Неразрушающие эффекты: тень ложится под слой, обводка — поверх
            // альфы, но под самими пикселями. Пиксели слоя не меняются.
            if layer.meta.fx.any() {
                let alpha: Vec<u8> = px.iter().skip(3).step_by(4).copied().collect();
                let fx = layer.meta.fx.clone();
                if fx.glow {
                    // Ореол идёт первым: он самый дальний от рисунка.
                    let g = crate::raster::layer_glow(
                        self.width,
                        self.height,
                        &alpha,
                        fx.glow_blur,
                    ...
                    crate::raster::over(out, self.width, self.height, &g, 1.0);
                }
                if fx.shadow {
            ...
            for i in 0..n {
                ...
                if let Some(b) = &clip {
                    a *= b.get(i).copied().unwrap_or(0) as f32 / 255.0;
                    if a <= 0.0 {
                        continue;
                    }
                }
...
mod tests {
    ...
    /// Композит документа 2×1: снизу цвет `under`, сверху `over` с режимом.
    fn blend_result(mode: BlendMode, over: [u8; 4], under: [u8; 4]) -> [u8; 4] {
        let mut d = Document::new(2, 1);
        d.layers[0].fill(under);
        let top = d.add_layer("верх");
        d.layers[top].fill(over);
    ...
    #[test]
    fn blend_modes_darken_lighten_and_difference() {
        // Затемнение берёт меньший из цветов, осветление — больший.
        let dark = blend_result(BlendMode::Darken, [80, 0, 0, 255], [200, 40, 0, 255]);
        assert_eq!(dark, [80, 0, 0, 255], "затемнение: {:?}", dark);
        let light = blend_result(BlendMode::Lighten, [80, 0, 0, 255], [200, 40, 0, 255]);
        assert_eq!(light, [200, 40, 0, 255], "осветление: {:?}", light);
        // Разность — модуль разности по каждому каналу.
        let diff = blend_result(BlendMode::Difference, [80, 0, 0, 255], [200, 40, 0, 255]);
        assert_eq!(diff, [120, 40, 0, 255], "разность: {:?}", diff);
        // Все пять режимов на месте в списке и названы по-русски.
        assert_eq!(BlendMode::ALL.len(), 10, "десять режимов");
        for m in BlendMode::ALL {
            assert!(!m.name().is_empty(), "у режима есть название");
        }
    }

    #[test]
    fn blend_color_dodge_and_burn_follow_the_backdrop() {
        // Осветление: средний src на тёмной подложке уходит в белый.
        let dodge = blend_result(BlendMode::ColorDodge, [160, 0, 0, 255], [64, 0, 0, 255]);
        assert!(dodge[0] > 200, "осветление высветлило: {:?}", dodge);
        // Затемнение: слабый src на средней подложке уходит в ноль.
        let burn = blend_result(BlendMode::ColorBurn, [40, 0, 0, 255], [120, 0, 0, 255]);
