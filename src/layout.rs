    ...
    for row in rows_for(app.tool) {
        match row {
            ...
            ParamRow::Checkbox { label, on } => {
                // У заливки это «ограничить область», у фигур и выделения —
                // «залить целиком»; читаем и пишем ровно тот флаг, который виден.
...
fn layers_panel(ui: &mut Ui, app: &mut App, r: Rect, actions: &mut Vec<Action>) {
    ...
    // Снизу панели живут две строки: свойства слоя (24 px) и кнопки (26 px).
