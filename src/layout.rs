    ...
    if let Some(t) = app.text.as_ref() {
        ...
        if app.params.vertical_text {
            ...
            for (i, ch) in t.buf.chars().enumerate() {
                let s = ch.to_string();
                let cw = ui.text_width(&s, size, bold);
                let base = p.1 + step * i as f32 + ui.fonts.ascent(size);
    ...
    for row in rows_for(app.tool) {
        match row {
            ...
            ParamRow::VerticalText => {
                let fr = [r[0] + PAD, y, r[2] - PAD, y + 20.0];
                let mut v = app.params.vertical_text;
                if ui.checkbox(fr, &mut v, "Вертикально") {
                    app.params.vertical_text = v;
                }
                y += 26.0;
            }
            ParamRow::Spacing => {
