...
fn layers_panel(ui: &mut Ui, app: &mut App, r: Rect, actions: &mut Vec<Action>) {
    ...
    let vis_rows = ((list_h / row_h).floor() as usize).max(1);
    // Активный слой всегда виден: список прокручивается к нему.
    let active_row = app.doc.layers.len() - 1 - app.doc.active;
    if active_row < app.layer_scroll {
        app.layer_scroll = active_row;
    } else if active_row >= app.layer_scroll + vis_rows {
        app.layer_scroll = active_row + 1 - vis_rows;
    }
    let max_scroll = app.doc.layers.len().saturating_sub(vis_rows);
    ...
    if let Some(from) = app.layer_drag {
        let target = (app.doc.layers.len() as i32
            - 1
            - (((ui.mouse.1 - list_top) / row_h).floor() as i32))
            .clamp(0, app.doc.layers.len() as i32 - 1) as usize;
        if ui.released {
            app.layer_drag = None;
            if target != from {
                app.doc.active = from;
    ...
    for k in app.layer_scroll..app.doc.layers.len() {
        ...
        if let Some(from) = app.layer_drag {
            let target = (app.doc.layers.len() as i32
                ...
                .clamp(0, app.doc.layers.len() as i32 - 1) as usize;
            if target != from {
                let line = (if target > from { list_top + (k as f32 + 1.0) * row_h } else { list_top + (k as f32) * row_h })
                    .clamp(list_top, list_top + list_h);
                ui.quad([rr[0], line - 1.5, rr[2], line + 1.5], theme::ACCENT);
            }
        }
...
fn status_bar(ui: &mut Ui, app: &mut App, w: f32, y: f32) {    ui.quad([0.0, y, w, y + STATUS_H], theme::PANEL);
