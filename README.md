...
| Текст | кегль, непрозрачность, жирный, вертикальный |
| Пипетка | — |
...
**Тон и насыщенность** (меню «Фильтры» → «Тон и насыщенность…») — сдвиг
оттенка (±180°), насыщенности и светлоты (±100 %) сразу по всем цветам или по
одному из шести секторов (красные, жёлтые, зелёные, голубые, синие,
пурпурные). У сектора есть «мягкий край»: у края диапазона оттенков влияние
слабее, а серый не принадлежит ни одному сектору и остаётся серым.

![Тон и насыщенность](img/hue_sat.png)

**Цветовой баланс** (меню «Фильтры» → «Цветовой баланс…») — девять регуляторов:
сдвиг красного, зелёного и синего отдельно в **тенях**, **средних тонах** и
**светах**. Полосы света считаются по светлоте пикселя и стыкуются плавно, так
что полутона не получают «ступенек».

![Цветовой баланс](img/balance.png)

**Привязка** (меню «Вид») — курсор притягивается к **направляющим**, к узлам
**сетки** и к шагу угла **15°** от начала мазка; каждая привязка включается и
выключается отдельно. Показывается подсказка: линия направляющей, рамка узла
сетки или линия от начала мазка. Привязка работает и когда ничего не рисуется,
и при перетаскивании выделения.
...
как ляжет в слой. Флажок **«Вертикально»** кладёт буквы столбиком: каждая
буква выровнена по центру колонки, шаг строки — знакомест шрифта, поэтому
колонка идёт ровно и без зазоров.

![Вертикальный текст](img/vertical_text.png)
...
| Открыть PSD | читает **слои** PSD: кадры, маски, папки, непрозрачность, видимость и режимы наложения |
| Экспорт в PSD | пишет **слои** PSD: кадр по содержимому, маска, папки-секции, непрозрачность, видимость, режимы и итоговая картинка |
...
![Импорт PSD со слоями](img/psd_layers.png)
...
(обычный, умножение, экран, наложение, добавить, затемнение, осветление,
разность, цветовое осветление и цветовое затемнение). Слой можно отразить по
...
![Режимы наложения](img/blend_modes.png)
...
симметрию мазка, навигатор, сдвиг оттенка по секторам, цветовой баланс,
окна коррекции и привязку к направляющим, сетке и углу, чтение и запись
слоёв PSD, новые режимы наложения и вертикальный текст.
...
**Тон и насыщенность.** Меню «Фильтры» → «Тон и насыщенность…»: сдвиг оттенка
(±180°), насыщенности и светлоты (±100 %) — либо сразу по всем цветам, либо по
одному из шести секторов, как в Photoshop. Сектор задаётся кнопками, у края
диапазона оттенков влияние ослабевает (мягкий переход между соседними
секторами), а серые пиксели не принадлежат ни одному сектору и потому не
красятся. Светлота в минус затемняет, в плюс осветляет.

![Тон и насыщенность](img/hue_sat.png)

**Цветовой баланс.** Меню «Фильтры» → «Цветовой баланс…»: девять регуляторов —
красный, зелёный и синий отдельно для **теней**, **средних тонов** и
**светов**. Полоса определяется светлотой пикселя, стыки считаются квадратично,
поэтому границы между полосами не видно. Оба окна используют общий с кривыми
предпросмотр: гистограмма слоя, правка видна сразу, в историю попадает один
шаг, «Отмена» возвращает слой целиком.

![Цветовой баланс](img/balance.png)

**Привязка.** В меню «Вид» появились три переключателя: привязка к
направляющим, привязка к сетке и шаг угла 15°. Курсор притягивается к
ближайшей направляющей (вертикальной или горизонтальной), к узлу сетки или к
ближайшему углу 15° от начала мазка — с расстоянием притяжения 6 экранных
пикселей, чтобы на любом масштабе ощущение было одинаковым. Подсказка рисуется
на холсте: малиновая линия направляющей, рамка узла сетки или линия от начала
мазка с кружком на конце. Работает и просто при наведении, и при перетаскивании
выделения.

![Привязка к направляющим](img/snap.png)

Всего 148 тестов (добавлены 2 на сдвиг оттенка по секторам, 2 на цветовой
баланс, 2 на окна и 3 на привязку).

## Что нового в 0.16.0

**PSD со слоями.** Главное ограничение прошлых релизов снято: «Файл → Открыть
PSD…» теперь разбирает слои файла, а не только итоговую картинку. Переносятся
кадр слоя (смещение относительно холста), каналы, **маска слоя**, непрозрачность,
видимость, режим наложения и **папки** — секции групп становятся нашими
папками, а служебные слои-границы не видны в панели. Имя слоя берётся из блока
`luni`, то есть кириллица и спецсимволы не ломаются.

Обратный путь тоже со слоями: «Экспорт в PSD…» пишет каждый слой своим кадром
(тесная рамка вокруг непрозрачного содержимого), маску, непрозрачность,
видимость и режим наложения, а папки — настоящими секциями групп. Если у
слоя-шапки папки есть своё содержимое, оно сохраняется отдельным слоем внутри
папки, а сама папка остаётся пустой, как в Photoshop. Режимы наложения, которых
у нас нет (цветовые, градиентные), заменяются обычным, и их число показывается
в строке состояния.

![Слои PSD](img/psd_layers.png)

**Пять новых режимов наложения.** Кроме обычного, умножения, экрана, наложения
и добавления теперь есть **затемнение**, **осветление**, **разность**,
**цветовое осветление** и **цветовое затемнение** — те самые, что чаще всего
встречаются в PSD. Цветовые считаются делением, как в Photoshop. Список
выпадает вверх, если вниз не помещается.

![Режимы наложения](img/blend_modes.png)

**Вертикальный текст.** У инструмента «Текст» появился флажок «Вертикально»:
буквы идут столбиком сверху вниз, каждая выровнена по центру колонки, шаг — по
знакоместу шрифта. Предпросмотр на холсте рисуется так же, как ляжет в слой.

![Вертикальный текст](img/vertical_text.png)

**Попутно исправлено:**
- выпадающий список режимов наложения и поле ввода имени файла не работали
  вовсе: идентификатор виджета менялся каждый кадр, поэтому открытый список
  закрывался сразу. Теперь id считается по положению виджета, а сам список
  рисуется поверх панелей;
- раскрытый список не перекрывался панелями, которые собираются позже, —
  для этого добавлен отдельный проход отрисовки;
- композит документа, собранного из файла (PSD, проект), мог остаться пустым
  и приводил к падению при первой перерисовке.

Всего 156 тестов (добавлены 4 на разбор и запись слоёв PSD, 2 на режимы
наложения, 1 на вертикальный текст и 1 на демо-обмен PSD).
